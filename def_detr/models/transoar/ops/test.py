...
def check_forward_equal_with_pytorch_half():
    ...
    attention_weights /= attention_weights.sum(-1, keepdim=True).sum(-2, keepdim=True) #normalize
    im2col_step = 2
    output_pytorch = ms_deform_attn_core_pytorch(value.half(), shapes, sampling_locations.half(), attention_weights.half()).detach().cpu()
    output_cuda = MSDeformAttnFunction.apply(value.half(), shapes, level_start_index, sampling_locations.half(), attention_weights.half(), im2col_step).detach().cpu()
    fwdok = torch.allclose(output_cuda, output_pytorch)
    max_abs_err = (output_cuda - output_pytorch).abs().max()
    max_rel_err = ((output_cuda - output_pytorch).abs() / output_pytorch.abs()).max()
