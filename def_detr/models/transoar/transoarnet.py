...
from .build import build_neck, build_pos_enc, build_backbone_monai
...
class TransoarNet(nn.Module):
    def __init__(self, args):
        ...
        self._input_level = args.input_level

        # Use auxiliary decoding losses if required
        self._aux_loss = args.aux_loss

        # Get backbone
        # self._backbone = build_backbone(args)
        self._backbone = build_backbone_monai(args)

        # Get neck
        self._neck = build_neck(args)
        ...
        self._query_embed = nn.Embedding(self.num_queries, hidden_dim * 2)   # 2 -> tgt + query_pos

        # Get positional encoding
        self._pos_enc = build_pos_enc(args)

        self._reset_parameter()

        self._seg_proxy = args.use_seg_proxy_loss
        if self._seg_proxy:
            in_channels = args.start_channels
            out_channels = 2 if args.fg_bg else args.num_organs + 1 # inc bg
